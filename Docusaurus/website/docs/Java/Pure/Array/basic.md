...
## 配列
### 宣言 + 初期化
Javaの配列は固定長配列である。宣言時に要素数を指定して、その容量を確保する。
```java
...
### 要素追加・削除
Javaの配列は固定長であるため、その長さを変更するような操作は基本的にできない。  
やるとするなら一例として、可変長であるListに変換して操作を行った後に配列へ再変換する方法がある。
...
注意点として、Listへの変換を単純な`Arrays.asList`だけで行った場合は**固定長のリスト**に変換する。  
...
### 配列の結合
Apache Commons Langライブラリの`ArrayUtils.addAll`を使用すれば可能。
